...
const schema = z.object({
  ...
  formaPagamento: z.coerce.number().int().min(0).max(2),
...
export default function OrderCreatePage() {
  ...
  const form = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: { pessoaId: 0, formaPagamento: 0, itens: [] }
  })
  ...
  const onSubmit = form.handleSubmit(async (d) => {
    ...
    form.reset({ pessoaId: 0, formaPagamento: 0, itens: [] })
  })
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField select fullWidth label="Forma de Pagamento" value={form.watch('formaPagamento')}
                onChange={(e)=>form.setValue('formaPagamento', Number(e.target.value))}>
