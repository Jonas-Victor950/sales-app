...
const FP_LABELS = ["Dinheiro", "Cartão", "Boleto"] as const;
const STATUS_LABELS = ["Pendente", "Pago", "Enviado", "Recebido"] as const;
export default function OrdersListPage() {
  const [status, setStatus] = React.useState<string | number>(""); // '' | 0..3
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        <Grid item xs={12} sm={3}>
          ...
          >
            ...
            <MenuItem value={3}>Recebido</MenuItem>
          </TextField>
        </Grid>
        <Grid item xs={12} sm={3}>
          <TextField
        ...
        <Card key={p.id} sx={{ mb: 1 }}>
          <CardContent>
            <Grid container>
              <Grid item xs={12} sm={8}>
                <Typography variant="subtitle1">
                  Pedido #{p.id} — Pessoa {p.pessoaId}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {new Date(p.dataVenda).toLocaleString()} •{" "}
                  {FP_LABELS[p.formaPagamento] ?? p.formaPagamento} •{" "}
                  {STATUS_LABELS[p.status] ?? p.status}• {p.status}
                </Typography>
              </Grid>
            ...
            <ul>
              {p.itens.map((i) => (
                <li key={i.id}>
                  {i.nomeProduto} — x{i.quantidade} • R${" "}
                  {i.valorUnitario.toFixed(2)} = R$ {i.subtotal.toFixed(2)}
                </li>
              ))}
            </ul>
          </CardContent>
